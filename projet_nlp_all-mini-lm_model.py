...
EXPORT_REL_PATH="CISI_results_BERT.REL"
SCORE_MIN = 0.6
POND_TFIDF = 0.2
POND_BERT = 0.8
...
def parse_documents(filepath):
    ...
    return documents #document est un dictionnaire {id_doc : texte_du_doc} pour tous les documents du dataset
...
#Pour ce modèle on a pas besoin de tockeniser
#on plonge les textes originaux non tockenisé dans la base vectorielle Sentence Trnasformers pour obtenir des embeddings de ces textes
doc_texts = [docs[doc_id] for doc_id in doc_ids] #liste de tous les textes originaux (non tokenisés) des documents dans l'ordre de doc_ids
...
def search(query_id):
    ...
    if not relevant_docs:
        relevant_docs = set(doc_ids)  #on prend tous les docs si aucun candidat trouvé parceque BERT peut trouver des similarités même si les termes de la query ne sont pas dans le vocabulaire

    # Embedding de la query
    query_text = queries[query_id]
    query_embedding = model.encode([query_text])

    # embeddings des docs candidats
    relevant_indices = [doc_ids.index(doc_id) for doc_id in relevant_docs]
    relevant_embeddings = doc_embeddings[relevant_indices]
    
    # ── Score TF-IDF (cosinus) ───
    query_tfidf_vec = vectorizer.transform([query_corpus[query_id]])
    candidate_tfidf_vecs = tfidf_matrix[relevant_indices]
    tfidf_scores = cosine_similarity(query_tfidf_vec, candidate_tfidf_vecs).flatten()

    # Similarité cosinus entre la query et les docs candidats
    similarities = cosine_similarity(query_embedding, relevant_embeddings).flatten()
    
    # Combinaison des scores TF-IDF et BERT
    tfidf_scores_norm = (tfidf_scores - tfidf_scores.min()) / (tfidf_scores.max() - tfidf_scores.min() + 1e-10)
    bert_scores_norm = (similarities - similarities.min()) / (similarities.max() - similarities.min() + 1e-10)
    combined_scores = POND_TFIDF * tfidf_scores_norm + POND_BERT * bert_scores_norm

    # Ranking
    ranked_docs = sorted(
        zip(relevant_docs, combined_scores),
        key=lambda x: x[1],
        reverse=True
    )
    
    ranked_docs = [(doc_id, score) for doc_id, score in ranked_docs if score > SCORE_MIN]

    return ranked_docs[:30]

#Dans cette version on utilise les embeddings de Sentence Transformers pour calculer la similarité entre la query et les documents candidats au lieu de la matrice TF-IDF.
#La matrice TF-IDF aura juste servi à faire le premier filtrage pour récupérer les documents candidats
